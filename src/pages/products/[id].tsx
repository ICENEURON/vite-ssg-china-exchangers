...
import { ManufacturerCard } from "../manufacturers/components/ManufacturerCard";
...
export default function ProductCategoryPage() {
    ...
    const categoryObj = t(`pages.products.categories.${id}`, { returnObjects: true }) as ProductCategory;
    const allManufacturers = t("pages.manufacturers.list", { returnObjects: true }) as Manufacturer[];

    // Fallback if the category doesn't exist
        ...
        return (
            <div className="min-h-screen flex items-center justify-center p-6 text-center">
                <div className="max-w-md">
                    ...
                    <Button asChild>
                        <Link to={addLanguageToPath("/products", currentLanguage)}>Return to Products</Link>
                    </Button>
                </div>
            </div>
        );
    ...
    // Filter manufacturers that are tagged with this product id
    const matchingManufacturers = allManufacturers.filter(m => 
        m.product_categories && m.product_categories.includes(categoryObj.id)
    );
        ...
        <>
            ...
            <main className="min-h-screen bg-background pb-20">
                ...
                <div className="max-w-6xl mx-auto px-6 md:px-12 mt-12 grid grid-cols-1 lg:grid-cols-12 gap-12">
                     ...
                     {/* Right Sidebar - Manufacturers List */}
                         ...
                         <div className="sticky top-28 bg-card border border-border/50 rounded-2xl p-6 shadow-xl">
                             ...
                             <div className="mt-8 pt-6 border-t border-border/50">
                                 ...
                                 <Button className="w-full bg-gradient-to-r from-primary to-orange-500 hover:opacity-90 transition-opacity font-bold shadow-md" asChild>
                                     <Link to={addLanguageToPath(`/rfq?product=${categoryObj.id}`, currentLanguage)}>
                                         Get Custom Quotes
                                     </Link>
                                 </Button>
                             </div>
