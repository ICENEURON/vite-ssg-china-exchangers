...
- 英文官网：`http://lsphe.lansland.com/sites/Satellite?c=Page&d=&rendermode=preview&cid=1484715242166&pagename=LS_ENHT%2FPage%2FLSHTIndex`
- 中文官网：`http://lsphe.lansland.com/sites/Satellite?c=Page&d=&rendermode=preview&cid=1484715192396&pagename=LS_HT%2FPage%2FHTIndex`
- 目标公司 slug：`lanzhou-lanshi-heat-exchange-equipment-co-ltd`
- 英文公司名（如果已知）：`Lanzhou Lanshi Heat Exchange Equipment Co. , Ltd.`
- 中文公司名（如果已知）：`兰州兰石换热设备有限责任公司`
...
- 产品图必须是方图或横图，宽度必须大于或等于高度
- 竖图不要下载，也不要写入 `product_assets`
- 如果某个产品仅有竖图可用，则该产品视为缺少合格产品图
...
21. 不要保留“只有产品名、SEO 或图片”的占位产品。产品至少要有一段真实的文字介绍：`short_description` 或 `full_description` 至少一项非空，且还需要至少一个补充内容块非空：`advantage`、`technical_parameters`、`details` 或真实 `product_docs` 其一满足。否则该产品必须从 `products[]` 中删除。
22. `seo_data` 不算产品详情，不能作为保留空产品的理由。
23. 只有真正公司级的文档才能放在 `company_docs`。凡是产品手册、产品样本、产品说明书、产品规格书，必须放到对应产品的 `product_docs`，并挂到该产品的 `product_assets` 中；如果不能明确匹配到某个已保留产品，则删除该文档，不要保留在 `company_docs`。
...
- `company_docs` 只放公司简介、企业画册、公司级白皮书等公司主体文档
- 产品手册、产品说明书、产品规格书、产品样本必须放入 `product_docs`
- 如果文档标题或文件名只能模糊对应多个产品，视为无法明确归属，删除，不要硬挂到某个产品
...
3. 只有满足内容门槛的真实产品才生成 `products[]` 条目。产品至少要有一段真实介绍文本：`short_description` 或 `full_description` 至少一项非空；同时还要有至少一个补充内容块非空：`advantage`、`technical_parameters`、`details` 或真实 `product_docs`。不满足则删除，不要为了凑产品数量保留空壳产品。
4. 如果官网存在产品页，就优先从产品页抽取：中英文名称、简介、详细介绍、优势、参数、FAQ/说明、视频、SEO 信息、产品图、认证图、PDF/DOC/DOCX 文档。
5. 如果某产品只有图片、产品名或 SEO 文案，没有可展示的正文介绍与细节内容，则不要保留该产品。
6. 产品图只保留方图或横图；竖图一律删除，不要进入最终 JSON。
7. 如果公司没有 YouTube 或社媒，就保留 `social_media_links: []` 和空 `video_link`。
8. 所有下载的文件必须放入正确目录，并确保 JSON 中引用到的每个 `file_name` 都真实存在于对应目录。
9. 如果官网图片是 webp、svg、png、jpg、jpeg 等格式，优先保留原始可用格式，除非下载环境必须转换。文档仅允许保留 pdf、doc、docx。
10. 下载后检查文件名是否包含乱码、空格或不可见字符；必要时重命名为标准 ASCII 文件名，并同步更新 JSON。
11. 如果某个 `company_docs` 文档实质上是产品手册或产品资料，且能明确对应到一个已保留产品，则把文件移到 `product_docs` 并挂到该产品；如果不能明确对应，则删除。
...
9. 最终保留的每个产品都不只是“名字 + SEO + 图片”，而是具备真实介绍文本和至少一类补充详情。
10. 最终 `product_assets` 中没有竖图，所有产品图都为方图或横图。
11. 产品相关文档已经放入对应产品的 `product_docs`；无法明确归属的产品文档已删除，没有留在 `company_docs`。
